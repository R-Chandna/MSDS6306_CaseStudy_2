...
JobLevelTable <- as.data.frame(table(df$JobLevel))
JobLevelTable <- data.frame(JobLevelTable[,-1], row.names = JobLevelTable[,1])
colnames(JobLevelTable) <- c("Freq")
...
ggplot(JobLevelTable, aes(reorder(x=row.names(JobLevelTable), -Freq), y=Freq)) + geom_bar(stat = "identity", aes(fill = row.names(JobLevelTable))) + labs(title = "Employees per Job Level", y = "Number of Employees", x = "") + theme(axis.text.x = element_text(angle = 15, hjust = 1), legend.position="none") + scale_fill_brewer(palette="Spectral")
